...
(function () {
    ...
    function addBackButton() {
        ...
        backButton.addEventListener('click', function () {
            // Try to go back to the blueprint with restore parameter
            if (document.referrer && document.referrer.includes('start-using-ai')) {
                window.history.back();
            } else {
                // Fallback: go to wizard page with restore parameter
                window.location.href = '/start-using-ai/?restore=true';
            }
        });
